...
c      program struct
      subroutine js(xbjfix,mq2fix,mq,fhad)
      implicit none
c declare all variables for easier debugging
      ...
      real*8 xbj, mq2, mq, m2, tf, scale, alphas, mq2l(4)
...
c of the calculation are written.
      open (unit = 11, file ='/userdisk/zeusqcd/develop/struct.dat',
     + status = 'old')
      open (unit = 12, file ='struct.out', status = 'unknown')
...
c      rewind 17
c      rewind 18
c for explanations of parameters see struct.dat
      read(11,*) ipdf
      read(11,*) ifch
      read(11,*) nborn
      read(11,*) ngcorr
...
c     call daind1
               ...
               write(12,100) xbj, mq2, scale, fhad 
               write(13,95) xbj, fhad
...
c     for the integral, the number adn the ESTimated error.
            ...
            xfhad = daind1(zmin,zmax,ffm,eps,2,max,kount,est)
            fhad = alphas*mq2/m2/4.d0/pi/pi*xfhad
            write(12,100) xbj, mq2, scale*scale, fhad
 30      continue
...
c   prop. to e_l^2
     ...
     #  4.5d0, 4.66666667d0, 4.83333333d0, 5d0/
      ...
      call locat(dlaeta,neta, dleta, ieta)
      call locat(dlaxi, nxi, dlxi, ixi)
...
c Longitudinal CF group structure: equation (13) in PLB347 (1995) 143 - 151
      ...
      threshf_l = 1.d0/6.d0/pi*xi*term1**3*beta*beta*pi*pi/2.d0
      return
      end
c Transverse CF group structure: equation (14) in PLB347 (1995) 143 - 151
      ...
      beta = dsqrt(eta/(1.d0 + eta))
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      threshf_t = 0.25d0/pi*term1*pi*pi/2.d0
      return
      end
c Transverse CA group structure: equation (16) in PLB347 (1995) 143 - 151
      ...
      beta = dsqrt(eta/(1.d0 + eta))
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      thresha_t = 0.25d0/pi*term1*(beta*(dlog(8.d0*beta*beta))**2
...
c Longitudinal CA group structure: equation (15) in PLB347 (1995) 143 - 151
      ...
      beta = dsqrt(eta/(1.d0 + eta))
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      thresha_l = 1.d0/6.d0/pi*xi*term1**3*beta**2*
...
c equation (17) in PLB347 (1995) 143 - 151
      ...
      beta = dsqrt(eta/(1.d0 + eta))
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      threshbar_l = 1.d0/6.d0/pi*xi*term1**3*beta**3*
     #     (-dlog(4.d0*beta*beta))
      return
      end
...
c equation (18) in PLB347 (1995) 143 - 151
      ...
      beta = dsqrt(eta/(1.d0 + eta))
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      threshbar_t = 0.25d0/pi*term1*beta*(-dlog(4.d0*beta*beta))
      return
      end
      ...
      parameter (neta = 73, nxi = 49)
      real*8 calcpts(neta, nxi), dlaeta(neta), dlaxi(nxi)
      real*8 eta, xi, dleta, dlxi, xsdqtb
      real*8 pxi, peta, f(-1:1), delxi, deleta
      integer ieta, ixi
      data (calcpts(j, 1), j = 1,neta) /0.1295D-10, 0.2306D-10,
     #    0.4104D-10, 0.7301D-10, 0.1299D-09, 0.2310D-09, 0.4108D-09,
     #    0.7305D-09, 0.1299D-08, 0.2310D-08, 0.4107D-08, 0.7305D-08,
     #    0.1299D-07, 0.2310D-07, 0.4105D-07, 0.7299D-07, 0.1298D-06,
     #    0.2306D-06, 0.4097D-06, 0.7275D-06, 0.1291D-05, 0.2289D-05,
     #    0.4053D-05, 0.7164D-05, 0.1262D-04, 0.2216D-04, 0.3863D-04,
     #    0.6678D-04, 0.1140D-03, 0.1911D-03, 0.3125D-03, 0.4940D-03,
     #    0.7466D-03, 0.1066D-02, 0.1424D-02, 0.1766D-02, 0.2031D-02,
     #    0.2187D-02, 0.2224D-02, 0.2165D-02, 0.2027D-02, 0.1833D-02,
     #    0.1606D-02, 0.1365D-02, 0.1130D-02, 0.9135D-03, 0.7238D-03,
     #    0.5639D-03, 0.4331D-03, 0.3286D-03, 0.2469D-03, 0.1839D-03,
     #    0.1361D-03, 0.1000D-03, 0.7314D-04, 0.5324D-04, 0.3858D-04,
     #    0.2787D-04, 0.2006D-04, 0.1440D-04, 0.1031D-04, 0.7363D-05,
     #    0.5248D-05, 0.3734D-05, 0.2651D-05, 0.1879D-05, 0.1330D-05,
     #    0.9404D-06, 0.6637D-06, 0.4680D-06, 0.3296D-06, 0.2319D-06,
     #    0.1629D-06/

      data (calcpts(j, 2), j = 1,neta) /0.1283D-10, 0.2284D-10,
     #    0.4063D-10, 0.7230D-10, 0.1286D-09, 0.2287D-09, 0.4068D-09,
     #    0.7236D-09, 0.1287D-08, 0.2288D-08, 0.4068D-08, 0.7235D-08,
     #    0.1286D-07, 0.2288D-07, 0.4067D-07, 0.7229D-07, 0.1285D-06,
     #    0.2283D-06, 0.4057D-06, 0.7205D-06, 0.1279D-05, 0.2268D-05,
     #    0.4015D-05, 0.7095D-05, 0.1250D-04, 0.2193D-04, 0.3825D-04,
     #    0.6614D-04, 0.1129D-03, 0.1893D-03, 0.3095D-03, 0.4893D-03,
     #    0.7394D-03, 0.1056D-02, 0.1410D-02, 0.1749D-02, 0.2013D-02,
     #    0.2166D-02, 0.2204D-02, 0.2144D-02, 0.2008D-02, 0.1817D-02,
     #    0.1592D-02, 0.1353D-02, 0.1119D-02, 0.9049D-03, 0.7170D-03,
     #    0.5586D-03, 0.4290D-03, 0.3255D-03, 0.2447D-03, 0.1823D-03,
     #    0.1348D-03, 0.9909D-04, 0.7245D-04, 0.5274D-04, 0.3822D-04,
     #    0.2760D-04, 0.1988D-04, 0.1426D-04, 0.1021D-04, 0.7295D-05,
     #    0.5199D-05, 0.3699D-05, 0.2627D-05, 0.1862D-05, 0.1318D-05,
     #    0.9315D-06, 0.6576D-06, 0.4636D-06, 0.3266D-06, 0.2297D-06,
     #    0.1614D-06/

      data (calcpts(j, 3), j = 1,neta) /0.1265D-10, 0.2252D-10,
     #    0.4007D-10, 0.7128D-10, 0.1268D-09, 0.2256D-09, 0.4011D-09,
     #    0.7134D-09, 0.1269D-08, 0.2256D-08, 0.4011D-08, 0.7132D-08,
     #    0.1268D-07, 0.2255D-07, 0.4010D-07, 0.7128D-07, 0.1267D-06,
     #    0.2252D-06, 0.4000D-06, 0.7104D-06, 0.1261D-05, 0.2235D-05,
     #    0.3959D-05, 0.6995D-05, 0.1233D-04, 0.2163D-04, 0.3773D-04,
     #    0.6520D-04, 0.1113D-03, 0.1866D-03, 0.3051D-03, 0.4824D-03,
     #    0.7290D-03, 0.1041D-02, 0.1391D-02, 0.1724D-02, 0.1985D-02,
     #    0.2136D-02, 0.2174D-02, 0.2115D-02, 0.1980D-02, 0.1791D-02,
     #    0.1569D-02, 0.1334D-02, 0.1104D-02, 0.8927D-03, 0.7072D-03,
     #    0.5510D-03, 0.4232D-03, 0.3212D-03, 0.2412D-03, 0.1797D-03,
     #    0.1330D-03, 0.9774D-04, 0.7146D-04, 0.5202D-04, 0.3770D-04,
     #    0.2723D-04, 0.1961D-04, 0.1407D-04, 0.1007D-04, 0.7195D-05,
     #    0.5129D-05, 0.3648D-05, 0.2591D-05, 0.1836D-05, 0.1300D-05,
     #    0.9189D-06, 0.6486D-06, 0.4574D-06, 0.3221D-06, 0.2267D-06,
     #    0.1593D-06/

      data (calcpts(j, 4), j = 1,neta) /0.1239D-10, 0.2205D-10,
     #    0.3924D-10, 0.6981D-10, 0.1242D-09, 0.2209D-09, 0.3929D-09,
     #    0.6987D-09, 0.1243D-08, 0.2209D-08, 0.3929D-08, 0.6985D-08,
     #    0.1242D-07, 0.2208D-07, 0.3927D-07, 0.6981D-07, 0.1241D-06,
     #    0.2205D-06, 0.3918D-06, 0.6957D-06, 0.1235D-05, 0.2190D-05,
     #    0.3878D-05, 0.6852D-05, 0.1207D-04, 0.2118D-04, 0.3694D-04,
     #    0.6387D-04, 0.1090D-03, 0.1828D-03, 0.2990D-03, 0.4725D-03,
     #    0.7142D-03, 0.1020D-02, 0.1362D-02, 0.1689D-02, 0.1944D-02,
     #    0.2093D-02, 0.2130D-02, 0.2072D-02, 0.1941D-02, 0.1757D-02,
     #    0.1537D-02, 0.1308D-02, 0.1082D-02, 0.8748D-03, 0.6932D-03,
     #    0.5399D-03, 0.4146D-03, 0.3147D-03, 0.2364D-03, 0.1761D-03,
     #    0.1303D-03, 0.9579D-04, 0.7004D-04, 0.5097D-04, 0.3694D-04,
     #    0.2669D-04, 0.1922D-04, 0.1379D-04, 0.9873D-05, 0.7052D-05,
     #    0.5027D-05, 0.3576D-05, 0.2540D-05, 0.1800D-05, 0.1274D-05,
     #    0.9004D-06, 0.6357D-06, 0.4482D-06, 0.3156D-06, 0.2220D-06,
     #    0.1560D-06/

      data (calcpts(j, 5), j = 1,neta) /0.1202D-10, 0.2139D-10,
     #    0.3807D-10, 0.6773D-10, 0.1205D-09, 0.2142D-09, 0.3810D-09,
     #    0.6777D-09, 0.1205D-08, 0.2143D-08, 0.3810D-08, 0.6776D-08,
     #    0.1205D-07, 0.2142D-07, 0.3809D-07, 0.6771D-07, 0.1204D-06,
     #    0.2139D-06, 0.3800D-06, 0.6749D-06, 0.1198D-05, 0.2124D-05,
     #    0.3760D-05, 0.6645D-05, 0.1171D-04, 0.2055D-04, 0.3584D-04,
     #    0.6195D-04, 0.1057D-03, 0.1773D-03, 0.2900D-03, 0.4584D-03,
     #    0.6928D-03, 0.9897D-03, 0.1322D-02, 0.1640D-02, 0.1887D-02,
     #    0.2031D-02, 0.2067D-02, 0.2012D-02, 0.1884D-02, 0.1704D-02,
     #    0.1493D-02, 0.1269D-02, 0.1051D-02, 0.8493D-03, 0.6729D-03,
     #    0.5241D-03, 0.4026D-03, 0.3056D-03, 0.2295D-03, 0.1710D-03,
     #    0.1265D-03, 0.9300D-04, 0.6800D-04, 0.4949D-04, 0.3587D-04,
     #    0.2591D-04, 0.1865D-04, 0.1339D-04, 0.9585D-05, 0.6846D-05,
     #    0.4879D-05, 0.3471D-05, 0.2465D-05, 0.1748D-05, 0.1237D-05,
     #    0.8742D-06, 0.6171D-06, 0.4352D-06, 0.3064D-06, 0.2155D-06,
     #    0.1515D-06/

      data (calcpts(j, 6), j = 1,neta) /0.1149D-10, 0.2046D-10,
     #    0.3639D-10, 0.6476D-10, 0.1152D-09, 0.2049D-09, 0.3644D-09,
     #    0.6480D-09, 0.1152D-08, 0.2049D-08, 0.3644D-08, 0.6478D-08,
     #    0.1152D-07, 0.2049D-07, 0.3642D-07, 0.6474D-07, 0.1151D-06,
     #    0.2046D-06, 0.3634D-06, 0.6453D-06, 0.1145D-05, 0.2031D-05,
     #    0.3596D-05, 0.6354D-05, 0.1120D-04, 0.1965D-04, 0.3426D-04,
     #    0.5923D-04, 0.1011D-03, 0.1695D-03, 0.2774D-03, 0.4385D-03,
     #    0.6627D-03, 0.9467D-03, 0.1264D-02, 0.1567D-02, 0.1806D-02,
     #    0.1944D-02, 0.1978D-02, 0.1925D-02, 0.1803D-02, 0.1632D-02,
     #    0.1429D-02, 0.1215D-02, 0.1006D-02, 0.8132D-03, 0.6442D-03,
     #    0.5019D-03, 0.3855D-03, 0.2925D-03, 0.2197D-03, 0.1638D-03,
     #    0.1211D-03, 0.8904D-04, 0.6512D-04, 0.4739D-04, 0.3435D-04,
     #    0.2481D-04, 0.1785D-04, 0.1282D-04, 0.9177D-05, 0.6555D-05,
     #    0.4672D-05, 0.3324D-05, 0.2360D-05, 0.1673D-05, 0.1184D-05,
     #    0.8372D-06, 0.5908D-06, 0.4167D-06, 0.2934D-06, 0.2064D-06,
     #    0.1451D-06/

      data (calcpts(j, 7), j = 1,neta) /0.1076D-10, 0.1916D-10,
     #    0.3408D-10, 0.6063D-10, 0.1079D-09, 0.1919D-09, 0.3413D-09,
     #    0.6068D-09, 0.1079D-08, 0.1919D-08, 0.3413D-08, 0.6068D-08,
     #    0.1079D-07, 0.1919D-07, 0.3411D-07, 0.6063D-07, 0.1078D-06,
     #    0.1916D-06, 0.3402D-06, 0.6042D-06, 0.1072D-05, 0.1902D-05,
     #    0.3368D-05, 0.5951D-05, 0.1048D-04, 0.1841D-04, 0.3209D-04,
     #    0.5547D-04, 0.9470D-04, 0.1589D-03, 0.2598D-03, 0.4107D-03,
     #    0.6209D-03, 0.8870D-03, 0.1185D-02, 0.1470D-02, 0.1692D-02,
     #    0.1822D-02, 0.1856D-02, 0.1806D-02, 0.1690D-02, 0.1530D-02,
     #    0.1341D-02, 0.1140D-02, 0.9436D-03, 0.7629D-03, 0.6045D-03,
     #    0.4708D-03, 0.3617D-03, 0.2745D-03, 0.2063D-03, 0.1536D-03,
     #    0.1137D-03, 0.8355D-04, 0.6109D-04, 0.4446D-04, 0.3222D-04,
     #    0.2328D-04, 0.1676D-04, 0.1203D-04, 0.8610D-05, 0.6150D-05,
     #    0.4383D-05, 0.3118D-05, 0.2214D-05, 0.1569D-05, 0.1111D-05,
     #    0.7854D-06, 0.5544D-06, 0.3909D-06, 0.2753D-06, 0.1936D-06,
     #    0.1361D-06/

      data (calcpts(j, 8), j = 1,neta) /0.9771D-11, 0.1739D-10,
     #    0.3095D-10, 0.5506D-10, 0.9794D-10, 0.1741D-09, 0.3097D-09,
     #    0.5510D-09, 0.9800D-09, 0.1743D-08, 0.3097D-08, 0.5510D-08,
     #    0.9795D-08, 0.1742D-07, 0.3096D-07, 0.5505D-07, 0.9786D-07,
     #    0.1739D-06, 0.3090D-06, 0.5487D-06, 0.9738D-06, 0.1726D-05,
     #    0.3057D-05, 0.5403D-05, 0.9522D-05, 0.1671D-04, 0.2914D-04,
     #    0.5038D-04, 0.8601D-04, 0.1442D-03, 0.2360D-03, 0.3731D-03,
     #    0.5642D-03, 0.8061D-03, 0.1077D-02, 0.1336D-02, 0.1539D-02,
     #    0.1658D-02, 0.1688D-02, 0.1643D-02, 0.1539D-02, 0.1393D-02,
     #    0.1221D-02, 0.1038D-02, 0.8592D-03, 0.6947D-03, 0.5504D-03,
     #    0.4289D-03, 0.3294D-03, 0.2499D-03, 0.1878D-03, 0.1399D-03,
     #    0.1035D-03, 0.7608D-04, 0.5563D-04, 0.4048D-04, 0.2936D-04,
     #    0.2120D-04, 0.1525D-04, 0.1095D-04, 0.7842D-05, 0.5601D-05,
     #    0.3992D-05, 0.2840D-05, 0.2016D-05, 0.1430D-05, 0.1012D-05,
     #    0.7152D-06, 0.5049D-06, 0.3560D-06, 0.2507D-06, 0.1764D-06,
     #    0.1240D-06/

      data (calcpts(j, 9), j = 1,neta) /0.8481D-11, 0.1509D-10,
     #    0.2686D-10, 0.4779D-10, 0.8500D-10, 0.1512D-09, 0.2689D-09,
     #    0.4782D-09, 0.8505D-09, 0.1512D-08, 0.2689D-08, 0.4782D-08,
     #    0.8502D-08, 0.1512D-07, 0.2688D-07, 0.4779D-07, 0.8493D-07,
     #    0.1509D-06, 0.2682D-06, 0.4763D-06, 0.8452D-06, 0.1499D-05,
     #    0.2654D-05, 0.4691D-05, 0.8265D-05, 0.1450D-04, 0.2529D-04,
     #    0.4374D-04, 0.7467D-04, 0.1252D-03, 0.2049D-03, 0.3240D-03,
     #    0.4901D-03, 0.7005D-03, 0.9363D-03, 0.1162D-02, 0.1339D-02,
     #    0.1443D-02, 0.1470D-02, 0.1431D-02, 0.1341D-02, 0.1214D-02,
     #    0.1064D-02, 0.9047D-03, 0.7488D-03, 0.6054D-03, 0.4797D-03,
     #    0.3738D-03, 0.2871D-03, 0.2178D-03, 0.1637D-03, 0.1220D-03,
     #    0.9021D-04, 0.6632D-04, 0.4849D-04, 0.3530D-04, 0.2558D-04,
     #    0.1848D-04, 0.1330D-04, 0.9547D-05, 0.6836D-05, 0.4883D-05,
     #    0.3480D-05, 0.2475D-05, 0.1758D-05, 0.1246D-05, 0.8820D-06,
     #    0.6236D-06, 0.4401D-06, 0.3104D-06, 0.2185D-06, 0.1538D-06,
     #    0.1081D-06/

      data (calcpts(j,10), j = 1,neta) /0.6889D-11, 0.1226D-10,
     #    0.2182D-10, 0.3882D-10, 0.6906D-10, 0.1228D-09, 0.2184D-09,
     #    0.3885D-09, 0.6909D-09, 0.1229D-08, 0.2186D-08, 0.3885D-08,
     #    0.6908D-08, 0.1228D-07, 0.2184D-07, 0.3882D-07, 0.6900D-07,
     #    0.1226D-06, 0.2178D-06, 0.3868D-06, 0.6867D-06, 0.1218D-05,
     #    0.2155D-05, 0.3810D-05, 0.6714D-05, 0.1178D-04, 0.2055D-04,
     #    0.3553D-04, 0.6068D-04, 0.1018D-03, 0.1665D-03, 0.2635D-03,
     #    0.3987D-03, 0.5701D-03, 0.7624D-03, 0.9468D-03, 0.1092D-02,
     #    0.1177D-02, 0.1199D-02, 0.1168D-02, 0.1095D-02, 0.9914D-03,
     #    0.8691D-03, 0.7392D-03, 0.6120D-03, 0.4949D-03, 0.3921D-03,
     #    0.3056D-03, 0.2346D-03, 0.1781D-03, 0.1338D-03, 0.9969D-04,
     #    0.7374D-04, 0.5421D-04, 0.3965D-04, 0.2885D-04, 0.2091D-04,
     #    0.1511D-04, 0.1087D-04, 0.7805D-05, 0.5588D-05, 0.3992D-05,
     #    0.2844D-05, 0.2024D-05, 0.1437D-05, 0.1019D-05, 0.7211D-06,
     #    0.5097D-06, 0.3597D-06, 0.2536D-06, 0.1787D-06, 0.1257D-06,
     #    0.8834D-07/

      data (calcpts(j,11), j = 1,neta) /0.5078D-11, 0.9039D-11,
     #    0.1608D-10, 0.2862D-10, 0.5090D-10, 0.9054D-10, 0.1610D-09,
     #    0.2863D-09, 0.5093D-09, 0.9057D-09, 0.1611D-08, 0.2863D-08,
     #    0.5091D-08, 0.9053D-08, 0.1610D-07, 0.2862D-07, 0.5087D-07,
     #    0.9039D-07, 0.1606D-06, 0.2851D-06, 0.5061D-06, 0.8975D-06,
     #    0.1590D-05, 0.2809D-05, 0.4950D-05, 0.8686D-05, 0.1515D-04,
     #    0.2621D-04, 0.4475D-04, 0.7507D-04, 0.1229D-03, 0.1946D-03,
     #    0.2945D-03, 0.4214D-03, 0.5639D-03, 0.7008D-03, 0.8087D-03,
     #    0.8722D-03, 0.8895D-03, 0.8669D-03, 0.8132D-03, 0.7366D-03,
     #    0.6459D-03, 0.5496D-03, 0.4551D-03, 0.3680D-03, 0.2916D-03,
     #    0.2272D-03, 0.1746D-03, 0.1325D-03, 0.9954D-04, 0.7416D-04,
     #    0.5486D-04, 0.4033D-04, 0.2949D-04, 0.2147D-04, 0.1556D-04,
     #    0.1124D-04, 0.8088D-05, 0.5805D-05, 0.4157D-05, 0.2969D-05,
     #    0.2117D-05, 0.1506D-05, 0.1069D-05, 0.7578D-06, 0.5364D-06,
     #    0.3792D-06, 0.2676D-06, 0.1887D-06, 0.1329D-06, 0.9350D-07,
     #    0.6570D-07/

      data (calcpts(j,12), j = 1,neta) /0.3246D-11, 0.5776D-11,
     #    0.1028D-10, 0.1828D-10, 0.3254D-10, 0.5787D-10, 0.1029D-09,
     #    0.1830D-09, 0.3255D-09, 0.5789D-09, 0.1029D-08, 0.1830D-08,
     #    0.3254D-08, 0.5786D-08, 0.1029D-07, 0.1828D-07, 0.3251D-07,
     #    0.5777D-07, 0.1026D-06, 0.1823D-06, 0.3236D-06, 0.5736D-06,
     #    0.1016D-05, 0.1796D-05, 0.3165D-05, 0.5553D-05, 0.9689D-05,
     #    0.1676D-04, 0.2862D-04, 0.4804D-04, 0.7868D-04, 0.1246D-03,
     #    0.1887D-03, 0.2703D-03, 0.3621D-03, 0.4506D-03, 0.5207D-03,
     #    0.5622D-03, 0.5739D-03, 0.5598D-03, 0.5255D-03, 0.4763D-03,
     #    0.4179D-03, 0.3557D-03, 0.2946D-03, 0.2382D-03, 0.1889D-03,
     #    0.1471D-03, 0.1130D-03, 0.8580D-04, 0.6447D-04, 0.4803D-04,
     #    0.3553D-04, 0.2612D-04, 0.1910D-04, 0.1390D-04, 0.1008D-04,
     #    0.7276D-05, 0.5238D-05, 0.3760D-05, 0.2693D-05, 0.1923D-05,
     #    0.1371D-05, 0.9750D-06, 0.6924D-06, 0.4908D-06, 0.3474D-06,
     #    0.2456D-06, 0.1734D-06, 0.1222D-06, 0.8609D-07, 0.6056D-07,
     #    0.4256D-07/

      data (calcpts(j,13), j = 1,neta) /0.1683D-11, 0.2996D-11,
     #    0.5329D-11, 0.9483D-11, 0.1687D-10, 0.3000D-10, 0.5335D-10,
     #    0.9489D-10, 0.1688D-09, 0.3002D-09, 0.5337D-09, 0.9491D-09,
     #    0.1688D-08, 0.3000D-08, 0.5334D-08, 0.9482D-08, 0.1686D-07,
     #    0.2995D-07, 0.5322D-07, 0.9451D-07, 0.1677D-06, 0.2975D-06,
     #    0.5268D-06, 0.9311D-06, 0.1641D-05, 0.2880D-05, 0.5027D-05,
     #    0.8694D-05, 0.1486D-04, 0.2495D-04, 0.4088D-04, 0.6478D-04,
     #    0.9827D-04, 0.1409D-03, 0.1892D-03, 0.2357D-03, 0.2727D-03,
     #    0.2951D-03, 0.3017D-03, 0.2946D-03, 0.2767D-03, 0.2511D-03,
     #    0.2205D-03, 0.1878D-03, 0.1556D-03, 0.1259D-03, 0.9978D-04,
     #    0.7776D-04, 0.5974D-04, 0.4535D-04, 0.3408D-04, 0.2540D-04,
     #    0.1878D-04, 0.1381D-04, 0.1010D-04, 0.7349D-05, 0.5327D-05,
     #    0.3848D-05, 0.2769D-05, 0.1988D-05, 0.1423D-05, 0.1017D-05,
     #    0.7247D-06, 0.5154D-06, 0.3660D-06, 0.2595D-06, 0.1836D-06,
     #    0.1298D-06, 0.9165D-07, 0.6462D-07, 0.4551D-07, 0.3201D-07,
     #    0.2250D-07/

      data (calcpts(j,14), j = 1,neta) /0.6419D-12, 0.1142D-11,
     #    0.2033D-11, 0.3617D-11, 0.6434D-11, 0.1144D-10, 0.2035D-10,
     #    0.3619D-10, 0.6437D-10, 0.1145D-09, 0.2035D-09, 0.3619D-09,
     #    0.6437D-09, 0.1145D-08, 0.2036D-08, 0.3617D-08, 0.6429D-08,
     #    0.1142D-07, 0.2030D-07, 0.3605D-07, 0.6399D-07, 0.1135D-06,
     #    0.2010D-06, 0.3552D-06, 0.6261D-06, 0.1099D-05, 0.1919D-05,
     #    0.3319D-05, 0.5676D-05, 0.9537D-05, 0.1565D-04, 0.2483D-04,
     #    0.3771D-04, 0.5420D-04, 0.7289D-04, 0.9107D-04, 0.1056D-03,
     #    0.1145D-03, 0.1173D-03, 0.1148D-03, 0.1080D-03, 0.9816D-04,
     #    0.8627D-04, 0.7352D-04, 0.6096D-04, 0.4934D-04, 0.3912D-04,
     #    0.3050D-04, 0.2343D-04, 0.1779D-04, 0.1337D-04, 0.9962D-05,
     #    0.7370D-05, 0.5418D-05, 0.3962D-05, 0.2883D-05, 0.2090D-05,
     #    0.1509D-05, 0.1087D-05, 0.7800D-06, 0.5585D-06, 0.3989D-06,
     #    0.2843D-06, 0.2022D-06, 0.1436D-06, 0.1018D-06, 0.7206D-07,
     #    0.5094D-07, 0.3596D-07, 0.2535D-07, 0.1785D-07, 0.1256D-07,
     #    0.8829D-08/

      data (calcpts(j,15), j = 1,neta) /0.1560D-12, 0.2778D-12,
     #    0.4943D-12, 0.8793D-12, 0.1565D-11, 0.2783D-11, 0.4949D-11,
     #    0.8801D-11, 0.1565D-10, 0.2783D-10, 0.4949D-10, 0.8801D-10,
     #    0.1565D-09, 0.2783D-09, 0.4947D-09, 0.8796D-09, 0.1563D-08,
     #    0.2778D-08, 0.4935D-08, 0.8766D-08, 0.1555D-07, 0.2759D-07,
     #    0.4887D-07, 0.8639D-07, 0.1523D-06, 0.2674D-06, 0.4670D-06,
     #    0.8085D-06, 0.1383D-05, 0.2327D-05, 0.3821D-05, 0.6073D-05,
     #    0.9248D-05, 0.1333D-04, 0.1799D-04, 0.2254D-04, 0.2625D-04,
     #    0.2854D-04, 0.2934D-04, 0.2878D-04, 0.2715D-04, 0.2472D-04,
     #    0.2177D-04, 0.1857D-04, 0.1541D-04, 0.1248D-04, 0.9900D-05,
     #    0.7719D-05, 0.5933D-05, 0.4505D-05, 0.3386D-05, 0.2523D-05,
     #    0.1866D-05, 0.1372D-05, 0.1003D-05, 0.7302D-06, 0.5294D-06,
     #    0.3824D-06, 0.2753D-06, 0.1976D-06, 0.1414D-06, 0.1010D-06,
     #    0.7200D-07, 0.5123D-07, 0.3637D-07, 0.2579D-07, 0.1826D-07,
     #    0.1290D-07, 0.9109D-08, 0.6423D-08, 0.4524D-08, 0.3183D-08,
     #    0.2237D-08/

      data (calcpts(j,16), j = 1,neta) /0.1961D-13, 0.3489D-13,
     #    0.6210D-13, 0.1105D-12, 0.1965D-12, 0.3495D-12, 0.6216D-12,
     #    0.1106D-11, 0.1967D-11, 0.3497D-11, 0.6218D-11, 0.1106D-10,
     #    0.1967D-10, 0.3497D-10, 0.6216D-10, 0.1105D-09, 0.1965D-09,
     #    0.3490D-09, 0.6203D-09, 0.1102D-08, 0.1956D-08, 0.3466D-08,
     #    0.6141D-08, 0.1086D-07, 0.1916D-07, 0.3363D-07, 0.5875D-07,
     #    0.1018D-06, 0.1743D-06, 0.2936D-06, 0.4832D-06, 0.7700D-06,
     #    0.1176D-05, 0.1703D-05, 0.2307D-05, 0.2907D-05, 0.3402D-05,
     #    0.3719D-05, 0.3837D-05, 0.3780D-05, 0.3579D-05, 0.3267D-05,
     #    0.2881D-05, 0.2463D-05, 0.2046D-05, 0.1659D-05, 0.1317D-05,
     #    0.1028D-05, 0.7899D-06, 0.5998D-06, 0.4509D-06, 0.3360D-06,
     #    0.2485D-06, 0.1828D-06, 0.1337D-06, 0.9729D-07, 0.7052D-07,
     #    0.5093D-07, 0.3666D-07, 0.2632D-07, 0.1884D-07, 0.1346D-07,
     #    0.9598D-08, 0.6828D-08, 0.4849D-08, 0.3438D-08, 0.2435D-08,
     #    0.1055D-08, 0.7219D-09, 0.4944D-09, 0.3380D-09, 0.2309D-09,
     #    0.1580D-09/

      data (calcpts(j,17), j = 1,neta) /0.9359D-15, 0.1667D-14,
     #    0.2964D-14, 0.5274D-14, 0.9381D-14, 0.1668D-13, 0.2967D-13,
     #    0.5277D-13, 0.9386D-13, 0.1669D-12, 0.2968D-12, 0.5279D-12,
     #    0.9386D-12, 0.1670D-11, 0.2967D-11, 0.5276D-11, 0.9378D-11,
     #    0.1667D-10, 0.2961D-10, 0.5261D-10, 0.9338D-10, 0.1656D-09,
     #    0.2934D-09, 0.5189D-09, 0.9154D-09, 0.1608D-08, 0.2811D-08,
     #    0.4874D-08, 0.8358D-08, 0.1410D-07, 0.2326D-07, 0.3721D-07,
     #    0.5712D-07, 0.8312D-07, 0.1134D-06, 0.1440D-06, 0.1697D-06,
     #    0.1868D-06, 0.1940D-06, 0.1920D-06, 0.1827D-06, 0.1675D-06,
     #    0.1483D-06, 0.1271D-06, 0.1057D-06, 0.8585D-07, 0.6820D-07,
     #    0.5324D-07, 0.4095D-07, 0.3111D-07, 0.2339D-07, 0.1743D-07,
     #    0.1290D-07, 0.9492D-08, 0.6945D-08, 0.5058D-08, 0.3668D-08,
     #    0.2651D-08, 0.1908D-08, 0.1085D-08, 0.7518D-09, 0.5207D-09,
     #    0.3591D-09, 0.2472D-09, 0.1705D-09, 0.1171D-09, 0.8025D-10,
     #    0.5496D-10, 0.3760D-10, 0.2595D-10, 0.1772D-10, 0.1209D-10,
     #    0.8250D-11/

      data (calcpts(j,18), j = 1,neta) /0.1082D-16, 0.1926D-16,
     #    0.3428D-16, 0.6099D-16, 0.1085D-15, 0.1929D-15, 0.3432D-15,
     #    0.6103D-15, 0.1085D-14, 0.1931D-14, 0.3432D-14, 0.6103D-14,
     #    0.1085D-13, 0.1931D-13, 0.3432D-13, 0.6101D-13, 0.1084D-12,
     #    0.1928D-12, 0.3425D-12, 0.6084D-12, 0.1080D-11, 0.1916D-11,
     #    0.3396D-11, 0.6006D-11, 0.1060D-10, 0.1865D-10, 0.3261D-10,
     #    0.5662D-10, 0.9729D-10, 0.1646D-09, 0.2726D-09, 0.4380D-09,
     #    0.6765D-09, 0.9922D-09, 0.1366D-08, 0.1752D-08, 0.2085D-08,
     #    0.2316D-08, 0.2427D-08, 0.2424D-08, 0.2322D-08, 0.2142D-08,
     #    0.1905D-08, 0.1639D-08, 0.1368D-08, 0.1113D-08, 0.8856D-09,
     #    0.6918D-09, 0.5317D-09, 0.4028D-09, 0.3011D-09, 0.2220D-09,
     #    0.1617D-09, 0.1166D-09, 0.8328D-10, 0.5904D-10, 0.4159D-10,
     #    0.2910D-10, 0.2028D-10, 0.1409D-10, 0.9774D-11, 0.6768D-11,
     #    0.4680D-11, 0.3231D-11, 0.2227D-11, 0.1532D-11, 0.1052D-11,
     #    0.7211D-12, 0.4935D-12, 0.3373D-12, 0.2304D-12, 0.1572D-12,
     #    0.1073D-12/

      data (calcpts(j,19), j = 1,neta) /0.1571D-19, 0.2795D-19,
     #    0.4973D-19, 0.8847D-19, 0.1573D-18, 0.2799D-18, 0.4978D-18,
     #    0.8855D-18, 0.1575D-17, 0.2801D-17, 0.4980D-17, 0.8855D-17,
     #    0.1575D-16, 0.2801D-16, 0.4978D-16, 0.8851D-16, 0.1573D-15,
     #    0.2796D-15, 0.4970D-15, 0.8829D-15, 0.1568D-14, 0.2781D-14,
     #    0.4930D-14, 0.8724D-14, 0.1541D-13, 0.2710D-13, 0.4749D-13,
     #    0.8261D-13, 0.1422D-12, 0.2415D-12, 0.4017D-12, 0.6500D-12,
     #    0.1012D-11, 0.1500D-11, 0.2091D-11, 0.2718D-11, 0.3281D-11,
     #    0.3695D-11, 0.3920D-11, 0.3959D-11, 0.3831D-11, 0.3562D-11,
     #    0.3191D-11, 0.2759D-11, 0.2311D-11, 0.1885D-11, 0.1503D-11,
     #    0.1176D-11, 0.9049D-12, 0.6860D-12, 0.5128D-12, 0.3783D-12,
     #    0.2759D-12, 0.1989D-12, 0.1421D-12, 0.1007D-12, 0.7086D-13,
     #    0.4961D-13, 0.3458D-13, 0.2403D-13, 0.1667D-13, 0.1154D-13,
     #    0.7980D-14, 0.5510D-14, 0.3798D-14, 0.2613D-14, 0.1794D-14,
     #    0.1229D-14, 0.8414D-15, 0.5753D-15, 0.3929D-15, 0.2681D-15,
     #    0.1829D-15/

      data (calcpts(j,20), j = 1,neta) /0.1092D-23, 0.1944D-23,
     #    0.3458D-23, 0.6153D-23, 0.1094D-22, 0.1947D-22, 0.3462D-22,
     #    0.6157D-22, 0.1095D-21, 0.1947D-21, 0.3464D-21, 0.6158D-21,
     #    0.1095D-20, 0.1947D-20, 0.3462D-20, 0.6156D-20, 0.1094D-19,
     #    0.1946D-19, 0.3458D-19, 0.6141D-19, 0.1091D-18, 0.1935D-18,
     #    0.3432D-18, 0.6076D-18, 0.1074D-17, 0.1891D-17, 0.3318D-17,
     #    0.5784D-17, 0.9990D-17, 0.1704D-16, 0.2851D-16, 0.4650D-16,
     #    0.7321D-16, 0.1100D-15, 0.1559D-15, 0.2063D-15, 0.2537D-15,
     #    0.2907D-15, 0.3137D-15, 0.3216D-15, 0.3156D-15, 0.2970D-15,
     #    0.2685D-15, 0.2340D-15, 0.1973D-15, 0.1616D-15, 0.1292D-15,
     #    0.1012D-15, 0.7800D-16, 0.5918D-16, 0.4426D-16, 0.3269D-16,
     #    0.2384D-16, 0.1719D-16, 0.1228D-16, 0.8703D-17, 0.6125D-17,
     #    0.4287D-17, 0.2988D-17, 0.2078D-17, 0.1441D-17, 0.9975D-18,
     #    0.6898D-18, 0.4763D-18, 0.3284D-18, 0.2259D-18, 0.1551D-18,
     #    0.1063D-18, 0.7273D-19, 0.4973D-19, 0.3396D-19, 0.2318D-19,
     #    0.1581D-19/

      data (calcpts(j,21), j = 1,neta) /0.8920D-30, 0.1589D-29,
     #    0.2826D-29, 0.5027D-29, 0.8941D-29, 0.1590D-28, 0.2829D-28,
     #    0.5031D-28, 0.8946D-28, 0.1591D-27, 0.2829D-27, 0.5031D-27,
     #    0.8946D-27, 0.1592D-26, 0.2829D-26, 0.5030D-26, 0.8942D-26,
     #    0.1590D-25, 0.2824D-25, 0.5019D-25, 0.8916D-25, 0.1583D-24,
     #    0.2808D-24, 0.4974D-24, 0.8795D-24, 0.1551D-23, 0.2726D-23,
     #    0.4764D-23, 0.8260D-23, 0.1416D-22, 0.2387D-22, 0.3932D-22,
     #    0.6273D-22, 0.9586D-22, 0.1387D-21, 0.1878D-21, 0.2364D-21,
     #    0.2775D-21, 0.3060D-21, 0.3202D-21, 0.3201D-21, 0.3063D-21,
     #    0.2808D-21, 0.2472D-21, 0.2100D-21, 0.1731D-21, 0.1390D-21,
     #    0.1093D-21, 0.8435D-22, 0.6410D-22, 0.4799D-22, 0.3545D-22,
     #    0.2586D-22, 0.1866D-22, 0.1333D-22, 0.9452D-23, 0.6653D-23,
     #    0.4658D-23, 0.3246D-23, 0.2256D-23, 0.1565D-23, 0.1084D-23,
     #    0.7494D-24, 0.5174D-24, 0.3565D-24, 0.2454D-24, 0.1685D-24,
     #    0.1155D-24, 0.7902D-25, 0.5402D-25, 0.3689D-25, 0.2518D-25,
     #    0.1718D-25/

      data (calcpts(j,22), j = 1,neta) /0.1096D-38, 0.1951D-38,
     #    0.3473D-38, 0.6177D-38, 0.1099D-37, 0.1954D-37, 0.3476D-37,
     #    0.6182D-37, 0.1099D-36, 0.1954D-36, 0.3477D-36, 0.6183D-36,
     #    0.1099D-35, 0.1954D-35, 0.3477D-35, 0.6182D-35, 0.1099D-34,
     #    0.1953D-34, 0.3473D-34, 0.6171D-34, 0.1096D-33, 0.1947D-33,
     #    0.3455D-33, 0.6123D-33, 0.1084D-32, 0.1914D-32, 0.3371D-32,
     #    0.5908D-32, 0.1029D-31, 0.1773D-31, 0.3015D-31, 0.5023D-31,
     #    0.8140D-31, 0.1269D-30, 0.1881D-30, 0.2619D-30, 0.3399D-30,
     #    0.4107D-30, 0.4658D-30, 0.5004D-30, 0.5127D-30, 0.5014D-30,
     #    0.4685D-30, 0.4187D-30, 0.3598D-30, 0.2990D-30, 0.2415D-30,
     #    0.1906D-30, 0.1477D-30, 0.1124D-30, 0.8430D-31, 0.6233D-31,
     #    0.4549D-31, 0.3285D-31, 0.2347D-31, 0.1665D-31, 0.1172D-31,
     #    0.8204D-32, 0.5719D-32, 0.3975D-32, 0.2757D-32, 0.1909D-32,
     #    0.1320D-32, 0.9116D-33, 0.6284D-33, 0.4323D-33, 0.2969D-33,
     #    0.2034D-33, 0.1392D-33, 0.9517D-34, 0.6500D-34, 0.4435D-34,
     #    0.3026D-34/

      data (calcpts(j,23), j = 1,neta) /0.1000D-51, 0.1780D-51,
     #    0.3168D-51, 0.5637D-51, 0.1003D-50, 0.1784D-50, 0.3173D-50,
     #    0.5641D-50, 0.1003D-49, 0.1785D-49, 0.3173D-49, 0.5643D-49,
     #    0.1003D-48, 0.1785D-48, 0.3173D-48, 0.5641D-48, 0.1003D-47,
     #    0.1784D-47, 0.3169D-47, 0.5634D-47, 0.1001D-46, 0.1779D-46,
     #    0.3157D-46, 0.5601D-46, 0.9923D-46, 0.1755D-45, 0.3096D-45,
     #    0.5444D-45, 0.9519D-45, 0.1652D-44, 0.2832D-44, 0.4777D-44,
     #    0.7872D-44, 0.1255D-43, 0.1913D-43, 0.2751D-43, 0.3698D-43,
     #    0.4629D-43, 0.5430D-43, 0.6026D-43, 0.6366D-43, 0.6406D-43,
     #    0.6137D-43, 0.5600D-43, 0.4890D-43, 0.4112D-43, 0.3351D-43,
     #    0.2662D-43, 0.2070D-43, 0.1581D-43, 0.1188D-43, 0.8795D-44,
     #    0.6426D-44, 0.4642D-44, 0.3320D-44, 0.2354D-44, 0.1658D-44,
     #    0.1161D-44, 0.8092D-45, 0.5625D-45, 0.3902D-45, 0.2702D-45,
     #    0.1869D-45, 0.1290D-45, 0.8894D-46, 0.6117D-46, 0.4200D-46,
     #    0.2878D-46, 0.1971D-46, 0.1347D-46, 0.9199D-47, 0.6279D-47,
     #    0.4283D-47/

      data (calcpts(j,24), j = 1,neta) /0.8260D-71, 0.1470D-70,
     #    0.2616D-70, 0.4655D-70, 0.8280D-70, 0.1473D-69, 0.2619D-69,
     #    0.4659D-69, 0.8285D-69, 0.1473D-68, 0.2620D-68, 0.4659D-68,
     #    0.8286D-68, 0.1473D-67, 0.2621D-67, 0.4659D-67, 0.8283D-67,
     #    0.1473D-66, 0.2619D-66, 0.4654D-66, 0.8273D-66, 0.1470D-65,
     #    0.2610D-65, 0.4633D-65, 0.8219D-65, 0.1456D-64, 0.2574D-64,
     #    0.4537D-64, 0.7967D-64, 0.1390D-63, 0.2406D-63, 0.4108D-63,
     #    0.6886D-63, 0.1123D-62, 0.1762D-62, 0.2627D-62, 0.3674D-62,
     #    0.4789D-62, 0.5843D-62, 0.6730D-62, 0.7377D-62, 0.7692D-62,
     #    0.7609D-62, 0.7139D-62, 0.6372D-62, 0.5448D-62, 0.4494D-62,
     #    0.3602D-62, 0.2818D-62, 0.2161D-62, 0.1629D-62, 0.1209D-62,
     #    0.8845D-63, 0.6395D-63, 0.4576D-63, 0.3247D-63, 0.2287D-63,
     #    0.1602D-63, 0.1117D-63, 0.7764D-64, 0.5385D-64, 0.3730D-64,
     #    0.2580D-64, 0.1780D-64, 0.1228D-64, 0.8445D-65, 0.5799D-65,
     #    0.3975D-65, 0.2721D-65, 0.1860D-65, 0.1270D-65, 0.8668D-66,
     #    0.5913D-66/

      data (calcpts(j,25), j = 1,neta) /0.9605D-99, 0.1710D-98,
     #    0.3042D-98, 0.5412D-98, 0.9627D-98, 0.1713D-97, 0.3045D-97,
     #    0.5416D-97, 0.9633D-97, 0.1713D-96, 0.3046D-96, 0.5418D-96,
     #    0.9633D-96, 0.1713D-95, 0.3046D-95, 0.5417D-95, 0.9633D-95,
     #    0.1713D-94, 0.3045D-94, 0.5413D-94, 0.9624D-94, 0.1710D-93,
     #    0.3039D-93, 0.5397D-93, 0.9581D-93, 0.1699D-92, 0.3009D-92,
     #    0.5319D-92, 0.9376D-92, 0.1646D-91, 0.2871D-91, 0.4959D-91,
     #    0.8445D-91, 0.1408D-90, 0.2274D-90, 0.3514D-90, 0.5127D-90,
     #    0.6988D-90, 0.8900D-90, 0.1069D-89, 0.1221D-89, 0.1327D-89,
     #    0.1366D-89, 0.1327D-89, 0.1220D-89, 0.1067D-89, 0.8955D-90,
     #    0.7267D-90, 0.5737D-90, 0.4428D-90, 0.3353D-90, 0.2495D-90,
     #    0.1828D-90, 0.1324D-90, 0.9484D-91, 0.6734D-91, 0.4744D-91,
     #    0.3324D-91, 0.2319D-91, 0.1612D-91, 0.1118D-91, 0.7745D-92,
     #    0.5356D-92, 0.3699D-92, 0.2550D-92, 0.1753D-92, 0.1204D-92,
     #    0.8255D-93, 0.5649D-93, 0.3862D-93, 0.2637D-93, 0.1800D-93,
     #    0.1228D-93/
...
c Longitudinal: equation (19) in PLB347 (1995) 143 - 151
      ...
      implicit none
      real*8 xi, pi, term1, fiim, fjjm
      parameter (pi = 3.14159265359d0)
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      asymp_l = 1.d0/6.d0/pi*(4.d0/xi - 4.d0/3.d0*term1 
     #     + (1.d0 - 2.d0/xi - 1.d0/6.d0*term1)*fjjm(xi)
     #     - (3.d0/xi + 0.25d0*term1)*fiim(xi))
      return
      end
c transverse: equation (20) in PLB347 (1995) 143 - 151
     ...
     #     + (7.d0/6.d0 + 1.d0/3.d0/xi + 1.d0/6.d0*term1)*fjjm(xi)
     #     + (1.d0 + 2.d0/xi + 0.25d0*term1)*fiim(xi))
      return
      end
c longitudinal mass factorization: equation (21) in PLB347 (1995) 143 - 151
      real*8 function asympbar_l(xi)
      implicit none
      real*8 xi, pi, term1, fjjm
      parameter (pi = 3.14159265359d0)
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      asympbar_l = 1.d0/6.d0/pi*(-6.d0/xi + 0.5d0*term1
     #     + (3.d0/xi + 0.25d0*term1)*fjjm(xi))
      return
      end
c transverse mass factorization: equation (22) in PLB347 (1995) 143 - 151
      real*8 function asympbar_t(xi)
      implicit none
      real*8 xi, pi, term1, fjjm
      parameter (pi = 3.14159265359d0)
      term1 = 1.d0/(1.d0 + 0.25d0*xi)
      asympbar_t = 1.d0/6.d0/pi*(4.d0/xi - 0.5d0*term1
     #     - (1.d0 + 2.d0/xi + 0.25d0*term1)*fjjm(xi))
      return
      end
c equation (24) in PLB347 (1995) 143 - 151
      real*8 function fiim(xi)
      implicit none
      real*8 pi, term1, term2, xi, dilog
      parameter (pi = 3.14159265359d0)
      term1 = dsqrt(xi)
      term2 = dsqrt(4.d0 + xi)
      fiim = 4.d0/term1/term2*(-pi*pi/6.d0 
...
c      call mrschm(xz,scale,1,upv,dnv,usea,dsea,str,chm,bot,glu)
      Q2=scale*scale
      upv=QPDFXQ('UPVAL',xz,Q2,IFLAG)
      dnv=QPDFXQ('DNVAL',xz,Q2,IFLAG)
      usea=QPDFXQ('UB',xz,Q2,IFLAG)
      dsea=QPDFXQ('DB',xz,Q2,IFLAG)
      str=QPDFXQ('SB',xz,Q2,IFLAG)
      chm=QPDFXQ('CB',xz,Q2,IFLAG)
      bot=QPDFXQ('BB',xz,Q2,IFLAG)
      glu=QPDFXQ('GLUON',xz,Q2,IFLAG)      
c      print*,'xz = ',xz
c      print*,'scale = ',scale
      gluexz = glu/xz
      if (nqcorr .ne. 0) then
...
c  assembles the appropriate partonic cross section for the structure function
     ...
     #           + sumelq5*xclqt
      ...
      double precision function alphadum(t)
c      DOUBLE PRECISION FUNCTION	ALPHA(T)
      IMPLICIT REAL*8(A-H,O-Z)
      COMMON/INPUT/alambda,flavor,qsct,qsdt,iord
      DATA PI/3.14159/
      DATA TOL/.0005/
