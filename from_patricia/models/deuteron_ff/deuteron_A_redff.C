...
void deuteron_A_redff() {
styling();
c = new TCanvas("c","Deuteron A Calcs",000,10,500,540);
plot_figure("deuteron_A_calc",gPad);
}

void plot_figure(TString fn,TPad *thePad,bool saxis=false,float ymin=0.0,float ymax=5.0,bool with_d=false) {
...
hr->SetYTitle("(1+Q^{2}/m_{0}^{2}) A^{1/2}");
...
TLegend *thlegend=new TLegend(0.40,0.74,0.63,0.84);
...
---------------------------------------------------------------------------------------*/
 ...
 Double_t fnorm = 0.0075*(1.+4./1.);
 for (Int_t i=0;i<nrna;i++) {
   qrna[i] = 1. + 0.05*i;
   arna[i] = fnorm/(1.+qrna[i]/1.);
 }

TGraph* gph=new TGraph(nrna,qrna,arna);
gph->SetLineColor(9);
...
*******************************************************************/
  ...
  for (Int_t i=0;i<npt24;i++) {
    apt24[i] = apt24[i]**(0.5)*(1.+qpt24[i]/xm0sq)*(1.+0.25*qpt24[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt29;i++) {
    ...
    apt29[i] = apt29[i]**(0.5)*(1.+qpt29[i]/xm0sq)*(1.+0.25*qpt29[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt25;i++) {
    ...
    apt25[i] = apt25[i]**(0.5)*(1.+qpt25[i]/xm0sq)*(1.+0.25*qpt25[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt28;i++) {
    ...
    apt28[i] = apt28[i]**(0.5)*(1.+qpt28[i]/xm0sq)*(1.+0.25*qpt28[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt23;i++) {
    ...
    apt23[i] = apt23[i]**(0.5)*(1.+qpt23[i]/xm0sq)*(1.+0.25*qpt23[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt27;i++) {
    ...
    apt27[i] = apt27[i]**(0.5)*(1.+qpt27[i]/xm0sq)*(1.+0.25*qpt27[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt22;i++) {
    ...
    apt22[i] = apt22[i]**(0.5)*(1.+qpt22[i]/xm0sq)*(1.+0.25*qpt22[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt26;i++) {
    ...
    apt26[i] = apt26[i]**(0.5)*(1.+qpt26[i]/xm0sq)*(1.+0.25*qpt26[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt21;i++) {
    ...
    apt21[i] = apt21[i]**(0.5)*(1.+qpt21[i]/xm0sq)*(1.+0.25*qpt21[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt32;i++) {
    ...
    apt32[i] = apt32[i]**(0.5)*(1.+qpt32[i]/xm0sq)*(1.+0.25*qpt32[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt31;i++) {
    ...
    apt31[i] = apt31[i]**(0.5)*(1.+qpt31[i]/xm0sq)*(1.+0.25*qpt31[i]/0.71)**4;
  ...
  for (Int_t i=0;i<npt30;i++) {
    ...
    apt30[i] = apt30[i]**(0.5)*(1.+qpt30[i]/xm0sq)*(1.+0.25*qpt30[i]/0.71)**4;
